    ...
    class Blueprint
    {
        ...
        public static Blueprint ImportBlueprintString(string bpString)
        {
            ...
            try
            {
                using (var msi = new MemoryStream(Convert.FromBase64String(bpString.Substring(1))))
                {
                    ...
                    {
                        using (var gs = new ZlibStream(msi, OfficeOpenXml.Packaging.Ionic.Zlib.CompressionMode.Decompress))
                        {
                            gs.CopyTo(mso);
                        }
        ...
        public string ExportBlueprintString()
        {
            ...
            using (var mso = new MemoryStream())
            {
                ...
                using (var gs = new ZLibStream(mso, System.IO.Compression.CompressionMode.Compress))
                {
                    msi.CopyTo(gs);
                }
